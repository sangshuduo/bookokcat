...
Bookokcat is a terminal-based EPUB reader with a split-view library and reader, full MathML and image rendering, automatic bookmarks, and inline annotations.
...
Bookokcat follows Vim-style keybindings throughout the interface for consistent, efficient navigation.
...
## Mouse Support
- Scroll with the wheel over either pane; Bookokcat batches rapid wheel events for smooth scrolling.
...
### Prerequisites
Bookokcat requires a C compiler/linker to be installed on your system for building dependencies.
...
2. Install bookokcat using Cargo:

```bash
cargo install bookokcat
```
