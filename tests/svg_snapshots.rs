use bookokcat::simple_fake_books::FakeBookConfig;
use bookokcat::test_utils::test_helpers::{
    create_test_app_with_custom_fake_books, create_test_terminal,
};
// SVG snapshot tests using snapbox
use bookokcat::{App, main_app::FPSCounter};
...
fn test_content_view_svg() {
    ...
    let mut app = App::new_with_config(Some("tests/testdata"), Some("/dev/null"), false);
...
fn test_chapter_title_normal_length_svg() {
    ...
    if let Some(book_info) = app.book_manager.get_book_info(1) {
        ...
        app.focused_panel = bookokcat::FocusedPanel::Main(bookokcat::MainPanel::Content);
        // Force animation to complete for testing
    }
...
fn test_mouse_event_batching_svg() {
    use bookokcat::event_source::{EventSource, SimulatedEventSource};
...
fn test_toc_navigation_bug_svg() {
    ...
    app.focused_panel = bookokcat::FocusedPanel::Main(bookokcat::MainPanel::NavigationList);
...
fn test_book_reading_history_with_many_entries_svg() {
    ...
    let temp_manager =
        bookokcat::test_utils::test_helpers::TempBookManager::new_with_configs(&book_configs)
    ...
    for i in 0..10 {
        ...
        let bookmark = bookokcat::bookmarks::Bookmark {
    ...
    let mut prod_bookmarks =
        bookokcat::bookmarks::Bookmarks::with_file(&bookmark_path.to_string_lossy());
    ...
    let mut app = bookokcat::App::new_with_config(
